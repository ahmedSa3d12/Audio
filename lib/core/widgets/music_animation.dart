...
class MusicAnimation extends StatefulWidget {
  const MusicAnimation({Key? key, required this.duration, required this.color})
      : super(key: key);
  final int duration;
  final Color color;
  @override
...
class _MusicAnimationState extends State<MusicAnimation>
  ...
  void initState() {
    ...
    final curveraniomation =
        CurvedAnimation(parent: animationController!, curve: Curves.easeInOut);
    animation = Tween<double>(begin: 0, end: 100).animate(curveraniomation)
      ..addListener(() {
        setState(() {});
      });
    ...
    return Row(
      ...
      children: new List<Widget>.generate(
          ...
          (index) => MusicAnimation(
              duration: duraton[index % 5], color: colors[index % 5])),
    );
  }
}
